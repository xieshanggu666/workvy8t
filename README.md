...
- **跨章奇遇链（规则 2.8.0）**：地图第 1、2 行各确定性放置一个「奇遇」节点（只转换
  休息/锻造/奖励、极端兜底转换一场遭遇/精英，**绝不覆盖商店**，也不改变其余节点的
  类型抽取与敌人 RNG 序列）。玩家在奇遇节点三选一并**立即承担代价**（生命/金币：
  先校验、失败零副作用可改选）；抉择写入随交接快照跨章继承的 `quests` 账本，
  后续章节按分支确定性触发**伏击战斗**（打赢经统一效果通道发链奖励、无普通战利品；
  战败即远征终结）或**自动奖励幕**（回写牌组/遗物/金币/生命），选「离开」类分支则
  链立即终结；另有按节点确定性选取、不跨章的本地小奇遇。`quest_choose` 是普通
  `/act` 动作，复用 request_id 幂等（双击只承担一次代价）、expected_rev 乐观锁与
  `BEGIN IMMEDIATE` 单事务原子提交；待抉择/伏击状态随存档续局保留；版本化回放识别
  旧 create 校验点形状（2.8.0 前无 quests），迁移点前按 legacy、越过后严格逐位比对，
  整章/整程回放只读隔离
...
**跨章奇遇链（固定槽位只转休息/锻造/奖励且绝不覆盖商店、节点抉择先扣代价且
未知选项/付不起代价零副作用、request_id 双击只扣一次、奖励幕自动结算回写牌组/遗物、
伏击战胜利发链奖励且无普通战利品/战败终结远征、链状态随 carry 跨章、续局保留待抉择、
旧 2.3 无 quests 形状的迁移点回放兼容、含伏击的合法两章流程逐位校验与整程只读隔离）**。

## API 摘要
- `POST /api/runs {seed?}` 建局
- `GET  /api/runs/{id}/resume` 续局
- `POST /api/runs/{id}/act {action,...}` 行动（choose_node / play / end_turn / claim_reward / forge / shop_buy / shop_remove / use_potion / discard_potion / companion_set_mode / commission_accept / commission_claim / quest_choose）
  - 奇遇抉择：`{action:"quest_choose", option:<选项下标>}`，代价（生命/金币）先校验后扣、
    效果（牌组/遗物/金币/治疗）经统一通道回写；非抉择节点/重复抉择 400、付不起代价 400（零副作用），
    request_id 双击只承担一次代价；伏击由 choose_node 进入奇遇节点时自动开战（用 play/end_turn 打）。
